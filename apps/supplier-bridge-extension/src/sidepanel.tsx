...
function App(): React.JSX.Element {
  ...
  if (!account) return <main className="shell">
    ...
    <form className="card" onSubmit={(event) => void login(event)}>
      ...
      <button className="button button--primary" disabled={busy !== null || !username.trim() || !password}>{busy === "login" ? copy("Prihlasujem…", "Přihlašuji…", "Signing in…") : copy("Prihlásiť", "Přihlásit", "Sign in")}</button>
    </form>
  </main>;
