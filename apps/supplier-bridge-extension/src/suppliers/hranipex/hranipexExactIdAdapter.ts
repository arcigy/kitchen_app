...
function extract(document: Document, context: ExactProductExtractionContext): SupplierProductLookupResult | null {
  const rows = [...document.querySelectorAll<HTMLTableRowElement>("tr.variantsTable-rowTop")];
  const topRow = context.requestedProductId === "__ARCIGY_CURRENT_PRODUCT__" && rows.length === 1
    ? rows[0] : rows.find((row) => cleanText(row).split(" ")[0] === context.requestedProductId);
...
export const hranipexExactIdAdapter: ExactIdSupplierAdapter = {
  ...
  extractExactProduct(document, context) { const result = extract(document, context); return result ? { ok: true, errorCode: null, result } : { ok: false, errorCode: context.requestedProductId === "__ARCIGY_CURRENT_PRODUCT__" ? "SUPPLIER_VARIANT_REQUIRED" : "SUPPLIER_PRODUCT_NOT_FOUND", result: null }; }
};
