...
def _pick_records(records: list[dict]) -> list[dict]:
    selected: list[dict] = []
    selected.extend([r for r in records if r.get("materialType") == "wood" and r.get("assetStatus") == "ready"][:4])
    selected.extend([r for r in records if r.get("assetStatus") == "fallback_asset"][:1])
    selected.extend([r for r in records if r.get("assetStatus") == "missing_asset"][:1])
    selected.extend([r for r in records if r.get("materialType") != "wood" and r.get("assetStatus") == "ready"][:2])
    by_id = {r["vendorDecorId"]: r for r in selected if isinstance(r.get("vendorDecorId"), str)}
    if len(by_id) < 8:
        for record in records:
            by_id.setdefault(record["vendorDecorId"], record)
            if len(by_id) >= 8:
                break
...
def build_scene(project_root: Path) -> None:
    ...
    selected = _pick_records(records)
    ...
    for i, record in enumerate(selected):
        ...
        mat = create_material_from_payload(payload, material_name=f"vendor_{record['vendorDecorId']}", project_root=str(project_root))
...
def main() -> int:
    ...
    out_blend = project_root / "blender" / "previews" / "vendor_material_mapping_preview.blend"
    out_png = project_root / "blender" / "previews" / "vendor_material_mapping_preview.png"
