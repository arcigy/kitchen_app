...
### Isolated develop health smoke 2026-07-17

After the canonical develop app's separate `dev` schema, object prefix, and persistent storage had already been proven, a deliberately small remote read-only smoke ran against its public URL. It used only `/health` and `/ready`: no login, project, catalog, module, save, or write endpoint. One virtual user ran for 20.152 seconds and completed 110/110 HTTP 200 requests with no failures. Aggregate p50/p95/p99/max latency was 48.976/80.543/154.154/166.806 ms; `/ready` p95 was 65.190 ms. This is a develop liveness/readiness observation, not a production, authenticated, project-open, catalog, capacity, spike, soak, or 1,000-user claim. Broader remote load steps remain gated by the progression below and by host/DB telemetry.
