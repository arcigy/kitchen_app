...
describe("wall join regression coverage", () => {
  ...
  test("regression_clipped_diagonal_outline_points_are_ordered", () => {
    ...
    expect(outlineArea(diagonal.outline)).toBeGreaterThan(0.4);
  ...
  test("regression_diagonal_wall_has_bottom_cut_cap_boundary", () => {
    ...
    expect(diagonal.a.source).toBe("cornerJoin");
    expect(diagonal.a.extra).toBeUndefined();
    expect(outlineHasSegment(diagonal.outline, diagonal.a.left, diagonal.a.right)).toBe(true);
  });
  ...
  test("regression_selected_outline_no_false_diagonal_closing_segment", () => {
    ...
    expect(outlineHasProperSelfIntersection(diagonal.outline)).toBe(false);
    expect(outlineHasSegment(diagonal.outline, diagonal.a.left, diagonal.a.right)).toBe(true);
    expect(outlineHasSegment(diagonal.outline, diagonal.b.left, diagonal.b.right)).toBe(true);
    expect(outlineHasSegment(diagonal.outline, diagonal.b.right, diagonal.outline[0]!)).toBe(true);
  });
  ...
  test("regression_no_open_diagonal_wall_end", () => {
    ...
    expect(diagonal.a.extra).toBeUndefined();
    expect(diagonal.b.extra).toBeUndefined();
    expect(outlineHasSegment(diagonal.outline, diagonal.a.left, diagonal.a.right)).toBe(true);
    expect(outlineHasSegment(diagonal.outline, diagonal.b.left, diagonal.b.right)).toBe(true);
  });
