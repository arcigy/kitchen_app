...
const DEFAULT_USER_PLACEHOLDER = "/organization/default-user.png";
...
export function organizationUserPhotoUrl(user: OrganizationUser | null): string | null {
  const value = user?.photoUrl.trim() ?? "";
  return value && value !== DEFAULT_USER_PLACEHOLDER ? value : null;
}
