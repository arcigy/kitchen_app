...
import type { EncryptedProjectFileEnvelope, ProjectSaveFile } from "./project-save-types";
import {
...
function createMaterialAssignments(catalog: ClientCatalog): ProjectMaterialAssignmentsState {
  const material = catalog.materials.find((item) => item.materialType === "board") ?? catalog.materials[0];
  const edge = catalog.materials.find((item) => item.materialType === "edge") ?? catalog.materials[0];
  const component = catalog.components[0];
  if (!material || !edge || !component) throw new Error("System catalog must include material, edge and component fixtures.");
  const snapshot = <T extends typeof material | typeof component>(definition: T) => ({
    definition,
    ...
    assignments: [
      {
        ...
        edgeOtherId: edge.id,
        thicknessMm: 18,
        customValues: { wastePercent: 7.5, note: "project override", nested: { enabled: true } },
        source: "user",
        snapshots: {
...
function encryptLegacyProjectSave(save: unknown, secret: string): EncryptedProjectFileEnvelope {
...
describe("project create/save/encryption", () => {
  ...
  it("assembles a complete serializable save with catalog snapshot", async () => {
    ...
    expect(save.phases[0].materialAssignments).toEqual(save.appState.materialAssignments);
    expect(save.catalogSnapshot.usedMaterialIds).toEqual(expect.arrayContaining([
      save.appState.materialAssignments.assignments[0].materialId,
      save.appState.materialAssignments.assignments[0].edgeFrontId
    ]));
    expect(save.catalogSnapshot.usedComponentIds).toContain(save.appState.materialAssignments.assignments[1].componentId);
    expect(save.catalogSnapshot.usedComponentIds).toEqual(expect.arrayContaining([
  ...
  it("saves and loads project snapshot under tenant storage", async () => {
    ...
    expect(loaded.appState.materialAssignments).toEqual(save.appState.materialAssignments);
  });
  ...
  it("preserves project material assignment snapshots through export and import", async () => {
    ...
    const envelope = await service.exportEncryptedProjectFile(ctxA, project.projectId, { secret: "test-project-file-secret" });
    const imported = await service.importEncryptedProjectFile(ctxA, envelope, { secret: "test-project-file-secret" });
    ...
    expect(imported.appState.materialAssignments.assignments[0].customValues).toEqual({
      wastePercent: 7.5,
  ...
  it("patches project materials without dropping unrelated save data", async () => {
    ...
    const base = assembleProjectSaveFile({
      ...
      assets: {
        bundled: [],
        external: [{ assetId: "system-texture", originalPath: "/system/texture.jpg", reason: "system_asset" }],
        missing: [],
        generated: [{ assetId: "render-keep", originalPath: "/renders/keep.png", kind: "render", reason: "regeneratable" }]
      }
    });
    base.catalogSnapshot.modules.push({ id: "module-snapshot-keep", marker: "preserve" });
    ...
    expect(patched.catalogSnapshot.modules).toEqual(base.catalogSnapshot.modules);
    ...
    expect(patched.catalogSnapshot.materials).toContainEqual(materialSnapshot.definition);
    expect(patched.catalogSnapshot.components).toContainEqual(componentSnapshot.definition);
    expect(patched.catalogSnapshot.usedMaterialIds).toContain(materialSnapshot.definition.id);
    expect(patched.catalogSnapshot.usedComponentIds).toContain(componentSnapshot.definition.id);
    if (materialSnapshot.unitPrice !== null) {
      expect((patched.catalogSnapshot.priceListSnapshot as { prices: Record<string, number> }).prices[materialSnapshot.definition.id]).toBe(materialSnapshot.unitPrice);
      expect(patched.catalogSnapshot.fullCatalog?.priceList.prices[materialSnapshot.definition.id]).toBe(materialSnapshot.unitPrice);
    }
  });
  ...
  it("serializes file material CAS updates and rejects stale full saves", async () => {
    ...
    const staleSave = assembleProjectSaveFile({
      ...
      assets: {
        bundled: [],
        external: [{ assetId: "keep-external", originalPath: "/system/keep", reason: "system_asset" }],
        missing: [],
        generated: []
      }
    });
    await repoA.saveProjectSnapshot(ctxA, project.projectId, project.activePhaseId, staleSave);
    ...
    const results = await Promise.allSettled([
      repoA.updateProjectMaterialAssignments(ctxA, project.projectId, project.activePhaseId, 0, nextA),
      repoB.updateProjectMaterialAssignments(ctxA, project.projectId, project.activePhaseId, 0, nextB)
    ]);

    expect(results.filter((result) => result.status === "fulfilled")).toHaveLength(1);
    const rejected = results.find((result): result is PromiseRejectedResult => result.status === "rejected");
    expect(rejected?.reason).toBeInstanceOf(ProjectMaterialRevisionConflictError);
    const loaded = await repoA.loadProjectSave(ctxA, project.projectId, project.activePhaseId);
    expect(loaded.appState.materialAssignments.revision).toBe(1);
    expect(loaded.phases[0].materialAssignments).toEqual(loaded.appState.materialAssignments);
    expect(loaded.assets).toEqual(staleSave.assets);
    expect(loaded.catalogSnapshot.modules).toEqual(staleSave.catalogSnapshot.modules);
    await expect(repoB.saveProjectSnapshot(ctxA, project.projectId, project.activePhaseId, staleSave))
      .rejects.toBeInstanceOf(ProjectMaterialRevisionConflictError);

    const sameRevisionForgery = structuredClone(loaded);
    sameRevisionForgery.appState.materialAssignments.assignments[0].customValues = { forged: "same-revision" };
    sameRevisionForgery.phases[0].materialAssignments = structuredClone(sameRevisionForgery.appState.materialAssignments);
    await expect(repoB.saveProjectSnapshot(ctxA, project.projectId, project.activePhaseId, sameRevisionForgery))
      .rejects.toBeInstanceOf(ProjectMaterialRevisionConflictError);

    const higherRevisionForgery = structuredClone(loaded);
    higherRevisionForgery.appState.materialAssignments.revision = 2;
    higherRevisionForgery.appState.materialAssignments.assignments[0].customValues = { forged: "higher-revision" };
    higherRevisionForgery.phases[0].materialAssignments = structuredClone(higherRevisionForgery.appState.materialAssignments);
    await expect(repoB.saveProjectSnapshot(ctxA, project.projectId, project.activePhaseId, higherRevisionForgery))
      .rejects.toBeInstanceOf(ProjectMaterialRevisionConflictError);
  });
  ...
  it("restores an older material assignment revision through the explicit version restore path", async () => {
    ...
    const restored = await service.restoreProjectVersion(ctxA, project.projectId, 1);
    ...
    expect((restored.appState.layout as { marker: string }).marker).toBe("version-one");
    const loaded = await service.loadProject(ctxA, project.projectId);
    expect(loaded.appState.materialAssignments.revision).toBe(initialAssignments.revision);
  });
  ...
  it("migrates version 1 saves before repository validation", async () => {
    ...
    for (const phase of legacy.phases as Array<Record<string, unknown>>) delete phase.materialAssignments;

    const migrated = loadProjectSaveFile(legacy, { clientId: ctxA.clientId, projectId: project.projectId });
    expect(migrated.saveFormatVersion).toBe(2);
    expect(migrated.appState.materialAssignments).toEqual(createEmptyProjectMaterialAssignmentsState());
    expect(migrated.phases[0].materialAssignments).toEqual(createEmptyProjectMaterialAssignmentsState());
    ...
    const loaded = await repo.loadProjectSave(ctxA, project.projectId, project.activePhaseId);
    expect(loaded.saveFormatVersion).toBe(2);
    expect(loaded.appState.materialAssignments.initialized).toBe(false);

    const legacyEnvelope = encryptLegacyProjectSave(legacy, "test-project-file-secret");
    const decrypted = decryptProjectSaveFile(legacyEnvelope, { secret: "test-project-file-secret" });
    expect(decrypted.saveFormatVersion).toBe(2);
    expect(decrypted.appState.materialAssignments).toEqual(createEmptyProjectMaterialAssignmentsState());
  });
  ...
  it("rejects invalid project material assignment state", async () => {
    ...
    expect(() => loadProjectSaveFile(phaseMismatch, { clientId: ctxA.clientId, projectId: project.projectId }))
  ...
  it("has no missing critical project save serializers", () => {
    ...
    expect(auditProjectSaveSerializers().covered).toContain("projectMaterialAssignments");
  });
