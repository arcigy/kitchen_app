...
function lineAtBackUnderside(box: THREE.Box3, y: number): LedStripPointMm[] {
  const z = box.max.z;
...
export function createAutomaticLedStripGroups(args: {
  ...
  for (const source of args.sources) {
    const tag = args.mode === "underUpper" ? "shelf" : args.mode === "plinthJoint" ? "plinth" : "shelf";
    const anchors = taggedMeshes(source.root, tag);
    ...
    const runs = anchors.flatMap((anchor, index) => {
      ...
      let points = lineAtBackUnderside(box, y);
