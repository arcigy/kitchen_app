...
export const snapshotSignature = (s: LayoutSnapshot) => {
  ...
  const w = s.walls
    .map((x) => `${x.id}:${x.params.aMm.x},${x.params.aMm.z}-${x.params.bMm.x},${x.params.bMm.z}:${x.params.thicknessMm}:${(x.params as any).justification ?? "center"}:${x.params.exteriorSign ?? 1}:${JSON.stringify(x.params.joinEnds ?? {})}`)
    .join("|");
