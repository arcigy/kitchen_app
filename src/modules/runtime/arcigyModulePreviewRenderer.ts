...
import { ARCIGY_MODULE_ICON_STYLE } from "../fwmFurniture/moduleIconRenderContract";
...
function cameraDirection(target: { cameraAzimuthDeg?: number }) {
...
function frameCamera(camera: THREE.PerspectiveCamera, root: THREE.Object3D, target: { cameraAzimuthDeg?: number }) {
  root.updateMatrixWorld(true);
...
/** Shared by catalog icon exports and live preset thumbnails. Owns only disposable preview roots. */
export function createArcigyModulePreviewRenderer() {
  ...
  return {
    render(root: THREE.Group, options: { cameraAzimuthDeg?: number } = {}) {
      const originalMaterials = new Set<THREE.Material>();
      root.traverse(object => {
        if (object instanceof THREE.Mesh) for (const material of Array.isArray(object.material) ? object.material : [object.material]) originalMaterials.add(material);
      });
      scene.add(root);
      try {
        const camera = new THREE.PerspectiveCamera(ARCIGY_MODULE_ICON_STYLE.cameraFovDeg, 1, 0.001, 100);
        frameCamera(camera, root, options);
        ...
        return { dataUrl: renderer.domElement.toDataURL("image/png"), hasTransparentBackground: corner[3] === 0 };
      } finally {
        scene.remove(root);
        disposeRoot(root);
